...
Fitky folder contains a simple .fit script for placing and fitting Gaussians on spectra as well as a more complex .lua script that places a peak in each given range and has an option to export file into a given folder. The ranges for peaks and range of interest can be configured quite easily based on the required situations.
